...
<head>
    ...
    <style>
        .sidebar-link {
            @apply flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-brand-green/10 hover:text-brand-green rounded-xl transition-all;
        }
        .sidebar-link.active {
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-white shadow-xl hidden lg:flex flex-col">
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('manager.approvals.pending') }}" class="sidebar-link {{ request()->routeIs('manager.approvals.pending') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('manager.approvals.expiring') }}" class="sidebar-link {{ request()->routeIs('manager.approvals.expiring') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @endphp
                    @if($expiringCount > 0)
                        <span class="mr-auto bg-orange-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $expiringCount }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('manager.businesses.index') }}" class="sidebar-link {{ request()->routeIs('manager.businesses.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('manager.owners.index') }}" class="sidebar-link {{ request()->routeIs('manager.owners.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('manager.governorates.index') }}" class="sidebar-link {{ request()->routeIs('manager.governorates.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('manager.districts.index') }}" class="sidebar-link {{ request()->routeIs('manager.districts.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('manager.sub-areas.index') }}" class="sidebar-link {{ request()->routeIs('manager.sub-areas.*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                    </svg>
                    {{ __('Sub Areas') }}
            ...
            <div class="p-4 border-t border-gray-100">
                <form method="POST" action="{{ route('manager.logout') }}">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 text-red-600 hover:bg-red-50 rounded-xl transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
